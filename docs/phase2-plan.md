...
1. **Mission execution reliability**
   ...
   - [x] Add and verify a lightweight deterministic dispatcher that polls explicit HERMES/AUTO work every 45 seconds, uses the atomic claim operation, records dispatcher/execution identity, validates result/debrief output, launches one temporary Hermes execution, and enforces database-backed global concurrency 1.
     ...
     - Production verification timestamps are recorded in `MISSION_CONTROL.md` and the AgentOS work log after each successful run `./scripts/deploy.sh` → live check.
   ...
   - [x] Add bounded stale-execution recovery and execution timeout.
     ...
     - Production re-verified 2026-07-21 21:19 MDT: additive schema sync and run `./scripts/deploy.sh` succeeded; health/state/missions HTTP 200; both systemd services enabled/active; concurrency remained 1; legacy worker remained preserved and disabled.
     - Final-source consistency fix re-verified 2026-07-21 22:12:16 MDT after confirming the active-Mission join and `FOR UPDATE OF e, m SKIP LOCKED`: fresh two-service run `./scripts/deploy.sh` passed; health/state/missions HTTP 200; PostgreSQL connected; both services enabled/active; dispatcher survived a full 45-second poll interval with the same MainPID and zero restarts; legacy worker disabled; active AUTO executions, production test markers, and controlled test processes all zero.
   - [x] Correct Mission Dispatch UX and mission-processor status truth.
     - New Mission exposes an explicit Manual or Automatic choice, defaults to Manual, explains both choices, and enforces MANUAL for agents without a supported dispatcher in both UI and API.
     ...
     - Eligible pending HERMES/MANUAL missions can be made Automatic in place. One guarded transaction changes the existing Mission and its existing queued attempt to AUTO without creating a duplicate mission or execution record; the unchanged atomic dispatcher path can then claim it.
     - Promotion reuses the same `supportsAutomaticDispatch` rule as creation and validates the agent inside the update transaction before either write. Unsupported agents receive HTTP 409 with both Mission and queued execution still MANUAL and unclaimable.
     ...
     - Isolated tests cover Manual default, explicit Automatic Hermes creation and claim, unsupported-agent enforcement, in-place Manual promotion, duplicate prevention, pending → active → awaiting-review lifecycle, truthful labels, and disabled legacy next-run suppression.
     - Execution-mode controls verified 2026-07-22 22:13 MDT: targeted mode-control and full deterministic dispatcher harnesses passed; TypeScript and production build passed; live omitted-mode Manual remained unclaimed; live explicit AUTO and promoted Manual were atomically claimed; repeated promotion returned 409 with one execution retained; exact test rows were cleaned; services remained enabled/active, concurrency 1, dispatcher up, and legacy worker disabled. Live Hermes omitted both required debriefs and preserved validation failed those executions; deterministic valid-debrief completion passed in the isolated harness.
     - Shared-agent-policy P1 correction verified 2026-07-22 22:19 MDT: targeted tests proved eligible Hermes promotion succeeds, writer-bot promotion returns 409 without changing either MANUAL row, and the rejected mission is never claimed. Full dispatcher tests, TypeScript, diff integrity, production build, and live creation/promotion checks passed; the exact live row was cleaned.
     - Test-isolation P1 correction verified 2026-07-22 22:27 MDT: the execution-mode harness now requires a separate empty `_test` database, rejects normal Mission Control before Prisma import, tracks and deletes only exact fixture IDs in `finally`, asserts zero remaining missions/executions, and creates no temporary database. Two consecutive targeted runs and the deterministic dispatcher suite passed and left zero fixture rows.
     - Authorization P1 correction verified 2026-07-22 22:41 MDT: Make Automatic now requires the existing `INTERNAL_API_SECRET` Bearer mechanism before Mission lookup or transaction, returns 401 for missing and 403 for invalid credentials without ID disclosure, and leaves both rows unchanged on rejection. The UI sends the operator-provided existing credential without persisting it. Isolated authorization/mode tests, dispatcher tests, TypeScript, diff integrity, production build, and controlled live unauthorized/authenticated checks passed; the exact live row was cleaned.
     - Verified 2026-07-21 22:46:57 MDT: isolated creation/label/banner tests and the full dispatcher harness passed; final run `./scripts/deploy.sh` passed; health/state/missions HTTP 200; dispatcher health up; legacy worker disabled; both services enabled/active; named mission unchanged.
