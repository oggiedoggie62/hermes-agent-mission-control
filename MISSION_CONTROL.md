...
The temporary Hermes prompt defines a mandatory debrief contract. Before reporting success, the child must write the exact absolute path, include non-empty `Summary`, `Work Performed`, `Evidence`, and `Decisions Made` level-two sections, and verify that exact file exists and is readable. Its stdout must consist only of a structured completion acknowledgement containing the exact Mission ID, execution ID, expected debrief path, and `Debrief Written: yes`. The dispatcher never trusts that acknowledgement by itself: completion still requires exit code zero, the exact acknowledgement, and an independently read and validated debrief at the predetermined path. Missing stdout, a missing or mismatched acknowledgement, a wrong path, an unreadable or malformed debrief, nonzero exit, launch failure, or configured runtime timeout fails both the durable attempt and compatibility Mission.

Contract-validation failures persist bounded child diagnostics with the execution error. Stdout and stderr are independently limited to 2,000-character excerpts, configured secret values and common credential forms are redacted before persistence, and empty streams are identified explicitly. The same bounded, redacted error is logged. Diagnostics improve failure investigation but never override the exact-path or section validators, fabricate an artifact, retry a mission, or change terminal lifecycle semantics.

Hermes Debrief Contract Hardening was verified in production on 2026-07-23 00:09 MDT. The required stop → build → restart sequence passed. The first controlled AUTO run failed closed and the new bounded diagnostics identified a pre-model runtime error: the systemd dispatcher PATH could not resolve the configured OpenAI Codex runtime. `codex` was installed at `/home/oggie/.npm-global/bin/codex`, so the dispatcher unit's explicit PATH was extended by only `/home/oggie/.npm-global/bin`, the installed unit was synchronized, systemd was reloaded, and only the dispatcher was restarted. The live dispatcher environment then resolved `codex-cli 0.145.0`.

A fresh harmless AUTO mission subsequently traversed pending/queued → active/claimed/running → completed/Awaiting Review. Its stdout exactly acknowledged the Mission ID, execution ID, absolute expected debrief path, and written state. The dispatcher independently read the expected 579-byte artifact and accepted all four non-empty required sections. Both disposable missions and their execution rows were deleted by exact ID; the successful disposable debrief was deleted by exact path; the failed attempt created no artifact; and production returned to zero active AUTO executions. Concurrency remained one, both systemd services remained enabled and active, and the legacy worker remained disabled.
...
`npm run test:debrief-contract` verifies the prompt and acknowledgement contracts, required debrief sections, exact-path acknowledgement matching, and bounded secret-redacted diagnostic formatting without invoking a model. `npm run test:dispatcher` refuses the normal Mission Control database. It requires `DISPATCHER_TEST_DATABASE_URL` to identify a separate, empty database whose name ends in `_test`. The controlled no-model-cost integration harness verifies recent claimed/running preservation, expired claimed/running recovery, two-process recovery uniqueness, capacity release and subsequent AUTO completion, a controlled Hermes timeout, queued-to-claimed-to-running transitions, global concurrency one, valid acknowledgement plus valid-debrief completion, acknowledgement without a file, file without a valid acknowledgement, wrong-path acknowledgement, malformed debrief, persisted bounded stdout/stderr diagnostics, immediate post-claim failure release, and deterministic child/row cleanup.
...
- Give the dispatcher an explicit minimal PATH containing project binaries, Hermes, the installed Codex runtime at `/home/oggie/.npm-global/bin`, and standard system binary directories
